...
public class DialogueManager {
	...
	public static void broadcastLocationChange(DungeonMasterResponse response, RoomContext context) {
		...
		if (newLocationUuid
				.equals(context.getRoomState().getLocationGraph().getCurrentLocation().getId())) {
			return;
		}
	 ...
	 * @param actionChoices
	 */
	public static void broadcastPlayerActions(DungeonMasterResponse.PlayerAction[] actionChoices,
			RoomContext context) {
		...
		for (UUID id : playerActions.keySet()) {
			...
			ArrayList<DungeonMasterResponse.PlayerAction> actions =
					playerActions.getOrDefault(id, new ArrayList<>());
			DungeonMasterResponse.PlayerAction[] playerActionsArray =
					actions.toArray(new DungeonMasterResponse.PlayerAction[actions.size()]);
			context.getBroadcastStrategy().sendToPlayer(new ShowPlayerActionsCommand(playerActionsArray),
					player);
		}
	}
	...
	public static void handleSwitchToCombat(String displayText, RoomContext context) {
		...
		// add all entities in the current location to the combat manager
		for (Entity entity : context.getRoomState().getLocationGraph().getCurrentLocation()
				.getEntities()) {
		...
		context.getBroadcastStrategy()
				.sendToAllPlayers(new CombatStartCommand(displayText, turnOrderIds));
	...
	/**
	 * End the current dialogue turn This transitions the RoomState from DIALOGUE_TURN to
	 * DIALOGUE_PROCESSING
	 * 
	 * @param context
	 */
	public static void endDialogueTurn(RoomContext context) {
		...
		for (Player player : context.getRoomState().getPlayers()) {
			...
			if (action == null) {
				context.getBroadcastStrategy()
						.sendToPlayer(new EndTurnCommand(context.getRoomState().getCurrentDialogueTurnNumber(),
								new SkillCheck.ActionResultDetail()), player);
				continue;
			}
			...
			dungeonMasterPrompt
					.append(player.getDisplayName() + " choses: " + actionResult.toString() + "\n");

			context.getBroadcastStrategy().sendToPlayer(
					new EndTurnCommand(context.getRoomState().getCurrentDialogueTurnNumber(), actionResult),
					player);
		}

		context.getBroadcastStrategy()
				.sendToBigScreen(new EndTurnCommand(context.getRoomState().getCurrentDialogueTurnNumber(),
						new SkillCheck.ActionResultDetail()));

		DungeonMasterResponse response =
				context.getDungeonMaster().handleDialogueTurn(dungeonMasterPrompt.toString());

		handleDungeonMasterResponse(response, context);
	}

	public static void handleDungeonMasterResponse(DungeonMasterResponse response,
			RoomContext context) {

		broadcastLocationChange(response, context);
		// RoomState roomState = context.getRoomState();
		if (response.switchToCombatThisTurn && context.getRoomState().getLocationGraph()
				.getCurrentLocation().getEntities().size() > 0) {
			...
			// // }
			// List<Entity> monster =
			// context.getRoomState().getLocationGraph().getCurrentLocation().getEntities();
	...
	// throw new IllegalArgumentException("Error creating monster: Invalid enum value for race or
	// entity class. " + e.getMessage());
	// } catch (NullPointerException e) {
	// throw new NullPointerException("Error creating monster: Missing required fields in enemy stats.
	// " + e.getMessage());
	// }
	// }
}
