...
public class CombatTurnActionHandler extends CommandHandler<CombatTurnActionCommand> {
	...
	public void handle(UUID playerId, CombatTurnActionCommand command, RoomContext context) {
		...
		if (context.getRoomState().getState() != RoomState.State.BATTLE) {
			throw new IllegalArgumentException(
					"Cannot process combat turn action when the room is not in battle state.");
		}
		...
		CombatResult result =
				combatManager.executeCombatTurn((Player) attacker, target, command.getAction());
		...
		context.getBroadcastStrategy()
				.sendToAllPlayers(new CombatEndTurnCommand(currentTurnIndex, result));
	...
	public static void handleNextTurn(RoomContext context) {
		...
		System.err.println("CombatTurnActionHandler.handleNextTurn: "
				+ combatManager.getCurrentTurnEntity().getDisplayName() + " is taking their turn.");

		while (combatManager.getCurrentTurnEntity().getAllegiance() == Allegiance.ENEMY) {

			combatManager.startTurn();
			context.getBroadcastStrategy()
					.sendToAllPlayers(new CombatStartTurnCommand(combatManager.getCurrentTurnIndex(),
							combatManager.getCurrentRoundIndex(), combatManager.getCurrentTurnEntity().getId()));
			...
			CombatResult monsterResult = monsterAI.handleMonsterTurn(combatManager.getParticipants(),
					combatManager.getCurrentTurnEntity());
			int currentTurn = combatManager.getCurrentTurnIndex();
			combatManager.endTurn();
			context.getBroadcastStrategy()
					.sendToAllPlayers(new CombatEndTurnCommand(currentTurn, monsterResult));
			try {
				Thread.sleep(2000);
			} catch (InterruptedException e) {
				e.printStackTrace();
	...
	public static void handleCombatOver(RoomContext context) {
		if (context.getCombatManager().isPlayersWin()) {
			...
			int totalXP = context.getCombatManager().getParticipants().stream()
					...
					.mapToInt(entity -> ((Monster) entity).getXpValue()).sum();
			...
			if (levelManager.checkLevelUp(context.getRoomState())) {
				...
				for (Player player : context.getRoomState().getPlayers()) {
					...
					context.getBroadcastStrategy().sendToPlayer(new SkillSelectionCommand(player.getId(),
							availableSkills, player.getSkillsInventory().getItems(), roomLevel), player);
				}
			}
